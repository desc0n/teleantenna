<div class="row">
	...
	<div class="col-sm-11">
		...
		<form action='/admin/writeoff/?writeoff=<?=$writeoff_id;?>' method='post' id='carryOutWriteoffForm'>
		<?}?>
            <h3 class="col-sm-3">Ответственный</h3>
            <h3 class="col-sm-7">
                <? if ($document_status == 1) {?>
                    <select class="form-control" id="writeoffContractor" name="writeoffContractor">
		...
		<?if($document_status == 1){?>
			<input type="hidden" name='carryOutWriteoffPost' value='<?=$writeoff_id;?>'>
			<button type="button" class='btn btn-success' id='carryOutWriteoffPost'>Провести</button>
		</form>
		<?}?>
	</div>
